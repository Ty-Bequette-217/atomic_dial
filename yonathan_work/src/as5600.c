...
static bool as5600_read_registers(uint8_t start_reg, uint8_t *buffer, size_t len) {
    int w = i2c_write_timeout_us(MAG_ENC_I2C_PORT, AS5600_I2C_ADDR, &start_reg, 1, true, 10000);
    if (w < 0) return false;
    int r = i2c_read_timeout_us(MAG_ENC_I2C_PORT, AS5600_I2C_ADDR, buffer, len, false, 10000);
    return r >= 0;
}
...
bool as5600_read_raw(uint16_t *raw_angle) {
    if (raw_angle == NULL) return false;
    uint8_t buf[2];
    if (!as5600_read_registers(AS5600_RAW_ANGLE_HI_REG, buf, 2)) return false;
...
bool as5600_read_degrees(float *degrees) {
    ...
    uint16_t raw_angle;
    if (!as5600_read_raw(&raw_angle)) return false;
