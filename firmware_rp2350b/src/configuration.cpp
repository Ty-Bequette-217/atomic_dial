...
bool Configuration::loadFromDisk() {
    ...
    LittleFSGuard fsGuard(logger_);
    if (!fsGuard.isMounted()) {
        return false;
    }
    ...
    size_t read = f.read(buffer_, sizeof(buffer_));
    f.close();

    pb_istream_t stream = pb_istream_from_buffer(buffer_, read);
    if (!pb_decode(&stream, &PB_PersistentConfiguration_msg, &pb_buffer_)) {
        char buf[200];
...
bool Configuration::saveToDisk() {
    ...
    if (!pb_encode(&stream, &PB_PersistentConfiguration_msg, &pb_buffer_)) {
        char buf[200];
